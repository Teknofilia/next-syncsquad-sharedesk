                      ...
                      <table className="relative mx-auto w-full whitespace-nowrap rounded-lg bg-white divide-y divide-gray-300 ">
                            <thead className="">
                                <tr className=" text-white text-left text- ">
                                    ...
                                    <th className="sticky top-0 font-semibold text-center bg-blue-800 text-sm uppercase px-6 py-4">  </th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-200  ">
                              <tr className={`text-gray-900 `}>
                                <td className="px-6 py-4 text-center uppercase whitespace-pre-line"> Ruangan Semeru </td>
                                <td className="px-6 py-4 text-center uppercase whitespace-pre-line"> {data?.role} </td>
                                <td className="px-6 py-4 text-center uppercase whitespace-pre-line"> {data?.toko} </td>
                                ...
                                {/* {data?.map((data,i)=>{                                 
                                    ...
                                    <tr key={i} className={`text-gray-900 `}>
                                        <td className="px-6 py-4 text-center uppercase whitespace-pre-line"> {data?.name} </td>
                                        <td className="px-6 py-4 text-center uppercase whitespace-pre-line"> {data?.role} </td>
                                        <td className="px-6 py-4 text-center uppercase whitespace-pre-line"> {data?.toko} </td>
                                        <td className="px-6 py-4 text-center uppercase whitespace-pre-line flex flex-row justify-center space-x-2">
                                            <FaEdit color={'blue'} onClick={()=>showModalUpdate(data.id,data)}/>
